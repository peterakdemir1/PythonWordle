...
def convertWord2Color(result):
    ...
    for letter in result:
        ...
        colored_letter = termcolor.colored(letter.character, "black", backgr)
...
def main():
    ...
    while wordle.stillAttempt():
        ...
        if len(attempt) != wordle.WORD_LENGTH:
            print(termcolor.colored(f"Word must be {wordle.WORD_LENGTH} characters long!", "red"), end = "")
        if attempt not in word_lst:
            print(termcolor.colored(f"{attempt} is not a valid word!", "red"), end="")
        
        wordle.attempt(attempt)
        displayResult(wordle)
    
    if wordle.gameWin():
        print(termcolor.colored("CORRECT. Congratulations!", "green"))
    else:
        print(termcolor.colored(f"WRONG. The answer is {word}!", "red"))
